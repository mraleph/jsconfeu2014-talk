...
Vector.understands = new Set(['x', 'x:', 'y', 'y:', 'add:', 'dot:', 'doesNotUnderstand:']);

Vector.prototype['doesNotUnderstand:'] = function (message) {
...
SendSite.prototype.tryRelink = function (self) {
  var ctor = self.constructor;
...
SendSite.prototype.doesNotUnderstand = function (self) {
  var ctor = self.constructor;
  if (typeof ctor === "function" &&
      ctor.understands instanceof Set &&
      ctor.understands.has("doesNotUnderstand:")) {
    var args = _.range(this.argc)
                .map(function (n) { return 'a' + n });
    this.link(
      CompileHandler({
        args: ['self'],
        op: 'self["doesNotUnderstand:"](' + ['"' + this.message + '"'].concat(args).join(', ') + ')',
        check: 'self.constructor === ctor'
      }, { ctor: ctor })
    )
    return true;
  }
  Object$doesNotUnderstand.apply(self, [this.message].concat(Array.prototype.slice.call(arguments, 1)));
};

SendSite.prototype.miss = function (self) {
  if (!this.tryRelink.apply(this, arguments)) {
    this.doesNotUnderstand(self);
  }

  return this.handler.apply(null, arguments);
};

function Object$doesNotUnderstand(message) {
  throw new Error("object " + this + " does not understand " + message);
}
