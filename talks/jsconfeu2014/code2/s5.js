...
Vector.methods = new Set(["add$1", "dot$1"]);
...
Load.prototype.miss = function (self) {
  ...
  throw new Error("object " + self + " has no field " + this.key);
};
...
Store.prototype.miss = function (self, value) {
  var ctor = self.constructor;
  if (typeof ctor === "function" &&
      ctor.fields instanceof Set &&
      ctor.fields.has(this.key)) {
    ...
    return this.handler(self, value);
  }
...
LoadIndexed.prototype.miss = function (self, idx) {
  var ctor = self.constructor;
...
Invoke.prototype.miss = function (self) {
  var ctor = self.constructor;
  if (typeof ctor === "function" &&
      ctor.methods instanceof Set &&
      ctor.methods.has(this.message.key + "$" + this.message.argc)) {
    var args = _.range(this.message.argc)
                .map(function (n) { return 'a' + n });
    this.link(
      CompileHandler({
  ...
  throw new Error("object `" + self + "` has no method " + this.key);
};
